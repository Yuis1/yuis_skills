[English](SKILL.md) | [简体中文](SKILL.zh-CN.md)

# Agent 开发

## 变更与评测

- Prompt、模型、Schema、工具和 Agent 编排变更必须版本化，并使用同一评测集比较收益、回归和成本。
- 需要语义理解的判断不得使用个案正则、关键词或固定话术伪装成模型决策；固定协议、标识符、安全硬约束和可审计确定性规则除外。
- 不要让 LLM 一次性输出大块代码、结构化内容或 SVG，优先使用 JSON + 模板的方式输出；若必须输出代码或大块结构化内容，则要进行多轮 React。

## 优化顺序

- 当 Agent 行为不理想时，不要增加规则代码进行修补和兜底，而应按以下顺序优化：
  1. 检查输入、上下文和任务边界。
  2. 建立复现样例和评测集。
  3. 优化 Prompt、Schema 和工具。
  4. 调整模型配置。
  5. 调整或增加编排节点，且必须论证其收益超过成本。

## Prompt 编写

- 使用角色可理解的业务语言。除非角色本身面向开发、运维或测试，否则不得泄露内部模块名、配置包名、实现名、链路名和调试语义。
- 在不改变语义和消息优先级的前提下优化 KV Cache。Prompt 按稳定程度排列：不变系统规则 → 低频上下文 → 中高频上下文 → 本轮输入。
- 废弃字段、恒为空输出和旧模型私有格式不得留在 Prompt 中；兼容逻辑由运行时代码处理。

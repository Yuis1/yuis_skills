...
- MRD、BRD、PRD、技术架构文档：要使用自然行文来叙述 TLDL、背景、问题痛点、调研、决策逻辑 等等内容，不能只有列表事项。对整体思路循序渐进展开表达，对生涩术语需要解释，表述专业地道，确保叙事连贯可读。
...
目标是让产品或工程负责人不读代码也能理解，同时节省输出。

- 首次提及内部组件时使用“业务含义（技术名，路径）”；通用概念、外部依赖和重复组件不反复附路径。
- 表述紧凑，不复述问题，不堆叠背景。
